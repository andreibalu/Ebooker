...
struct ReadingStats: Equatable {
    ...
    /// Synthetic stats used only by the onboarding "Reading Stats" spotlight so the user sees
    /// a populated-looking activity card before they've actually listened to anything. Not
    /// persisted; recomputed on each access so the heatmap always ends on today.
    ///
    /// Spans 120 days so the card picks the `.fourMonth` heatmap scale ("Last 4 months", 18-week grid).
    static var onboardingPreview: ReadingStats {
        let calendar = Calendar.current
        let today = calendar.startOfDay(for: .now)
        // Deterministic but varied minute pattern. Index 0 = today; increasing index = older.
        // Leading 5 entries non-zero ⇒ 5-day current streak ending today. A 30-day seed pattern
        // is repeated 4× with a phase shift so the 4-month heatmap doesn't look obviously cyclic.
        let seed: [Int] = [
            22, 18, 31, 14, 26,
            0, 41, 28, 0, 19,
            34, 22, 0, 16, 27,
        ...
        let perDayMinutes: [Int] = (0..<120).map { offset in
            // First 5 days come straight from the seed to preserve the streak.
            if offset < 5 { return seed[offset] }
            // Older days: phase-shifted seed values to avoid a visibly repeating pattern.
            let i = (offset + (offset / 30) * 7) % seed.count
            return seed[i]
        }

        let mockBookID = UUID(uuidString: "00000000-0000-0000-0000-000000000001") ?? UUID()
        let mockTitle = "Pride and Prejudice"
        let mockAuthor = "Jane Austen"
        ...
        for (offset, minutes) in perDayMinutes.enumerated() where minutes > 0 {
            guard let day = calendar.date(byAdding: .day, value: -offset, to: today) else { continue }
            let dayStart = calendar.startOfDay(for: day)
            let key = ReadingSession.makeDayKey(date: dayStart, calendar: calendar)

            activityByDay[key] = ReadingDayActivity(
                dayKey: key,
                date: dayStart,
                minutes: minutes,
                topBookID: mockBookID
            )
            totalMinutes += minutes
            firstDay = min(firstDay, dayStart)

            // Concentrate listening in plausible evening hours (19–21) and weekday mornings (8).
            let evenings = [19, 20, 21]
            let hour = evenings[offset % evenings.count]
            hourMinutes[hour] += minutes

            let weekday = calendar.component(.weekday, from: dayStart) // 1 = Sunday
            let mondayFirst = (weekday + 5) % 7
            dowMinutes[mondayFirst] += minutes
            ...
            if bestDay == nil || minutes > (bestDay?.minutes ?? 0) {
                bestDay = BestDay(
                    ...
                    minutes: minutes,
                    topBookID: mockBookID,
                    topBookTitle: mockTitle,
                    topBookAuthor: mockAuthor
                )
            }
        }
        ...
        let totalSessions = activityByDay.count
        let daysTracked = max(1, (calendar.dateComponents([.day], from: firstDay, to: today).day ?? 0) + 1)

        return ReadingStats(
            totalMinutes: totalMinutes,
            totalSessions: totalSessions,
            avgSession: totalSessions > 0 ? Double(totalMinutes) / Double(totalSessions) : 0,
            bestDay: bestDay,
            bestHour: bestHour,
            bestDow: bestDow,
            ...
            longestStreak: 5,
            booksFinished: 0,
            topAuthor: mockAuthor,
            topAuthorMinutes: totalMinutes,
            bookMinutes: [mockBookID: totalMinutes],
            longestBookID: mockBookID,
            longestBookTitle: mockTitle,
            longestBookAuthor: mockAuthor,
            longestBookMinutes: totalMinutes,
            freeMinutes: 0,
            freePct: 0,
