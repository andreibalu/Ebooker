...
1. **LibriVox catalog (primary, iPhone)** — 20,000+ books from librivox.org. Cached locally in `LibriVoxBook` via `LibriVoxCatalogSync` (24h incremental sync). UI is the "Free Books" tab in `ContentView`, driven by `BrowseLibriVoxView` → `LibriVoxBookDetailView`. Supports:
   ...
   - **Classics-first load order.** `BrowseLibriVoxViewModel.triggerSyncIfNeeded` awaits `performPreload` (curated `curatedClassicIDs` via `LibriVoxAPIClient.fetchBooks(ids:)` → `LibriVoxCatalogSync.seed`) **before** kicking off the full `performSync`. The blocking overlay (`isInitialLoading`) only shows while there is nothing on screen yet; once classics appear the full 20k sync continues behind a non-blocking inline banner (`isLoadingFullCatalog`). Don't run the full sync ahead of (or concurrently blocking) the preload.
   - **Hardened API client (don't surface raw `DecodingError`).** `LibriVoxAPIClient` never lets "The data couldn't be read because it isn't in the correct format." reach the UI. The feed routinely returns an `{"error":"…"}` body with HTTP 200 (no match) or an HTML page (server hiccup); `fetchData` validates HTTP status (→ `LibriVoxAPIError.serverUnavailable`) and the lenient `decode` helper maps the error-envelope/empty case to `[]` and anything unparseable to `LibriVoxAPIError.unreadableResponse`. `URLError` is deliberately **not** wrapped so `isNetworkUnavailable` offline classification still works. `LibriVoxCatalogSync.fetchPageWithRetry` retries transient page failures (3 attempts, linear backoff) so one flaky page can't abort the whole sync.
   - **Offline/empty states** (`BrowseLibriVoxView`): `isOfflineWithNoData` / `isOfflineWithCachedData` / `loadFailedWithNoData` are all featured-aware (cached classics count as "has data"), driving a no-internet state, an "Offline — showing saved books" banner, or a retriable "Couldn't Load Free Books" state respectively.
...
- **Preference controls bind live to the app's existing `@AppStorage` keys** (`resumeBacktrackSeconds`, `skipBackSeconds`, `skipForwardSeconds`, `momentBacktrackSeconds`) — so persistence is automatic, the scene-6 summary shares one source of truth, and the choice is freely reversible. "Open Library" calls `onboarding.complete()` and routes into the chosen home tab (free → Free Books, own → All Books). The **"Free books" choice persists** via `@AppStorage("startOnFreeBooks")`: when true, the app launches on Free Books on **every** launch (a one-time `.onAppear` guard in `ContentView` applies it for relaunched users) **and** reorders the library tabs to Favorites / Free Books / All Books in both `tabPicker` and the swipeable `libraryContent` (driven by the `tabOrder` computed property). Choosing "My books" (and all legacy users) leaves `startOnFreeBooks=false` — unchanged behavior: lands on Favorites, tab order Favorites / All Books / Free Books.
